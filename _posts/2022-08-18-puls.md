...
seo_title: 'Hvad er en normal puls? Se grænser, alder & måling'
description: 'Hvad er en normal puls i hvile og under træning? Lær om hjertets minutvolumen, slagvolumen, maxpuls, og se hvordan du måler din puls korrekt.'
permalink: /puls/
language: da
header:
  ...
  caption: Hvad er normal puls - alt om hjertets rytme
categories:
- Fysiologi
tags:
- puls
- træningslære
last_modified_at: '2026-08-03T23:25:00Z'
toc: 'true'
faq:
- question: Hvad er en normal hvilepuls?
  answer: For raske voksne ligger en normal hvilepuls typisk mellem 60 og 100 slag pr. minut (BPM). Veltrænede personer og udholdenhedsatleter kan have en hvilepuls helt ned på 30–50 BPM.
- question: Hvorfor ændrer pulsen sig gennem dagen?
  answer: Pulsen tilpasser sig løbende kroppens iltbehov. Den stiger ved fysisk aktivitet, stress, fordøjelse, koffein og varme, og den falder under hvile, afslapning og søvn.
- question: Hvad er forskellen på puls og blodtryk?
  answer: Pulsen udtrykker hvor mange gange hjertet slår pr. minut. Blodtrykket er det tryk, som blodet udøver mod væggene i dine pulsårer (arterier), når hjertet pumper.
---
...
> ### ⏱️ Kort fortalt: Hvad er en normal puls?
> * **Normal hvilepuls (voksne):** **60 – 100 slag/min** (atleter: 30–50 slag/min).
> * **Normal hvilepuls for mænd:** Ca. **60 – 75 slag/min**.
> * **Normal hvilepuls for kvinder:** Ca. **65 – 78 slag/min** (lidt højere pga. mindre hjertevolumen).
> * **Maxpuls:** Falder naturally med alderen (estimeres ofte som ca. $220 - \text{alder}$).
> * **Rytme:** Hjertet skal slå med en regelmæssig og stabil rytme i hvile.
...
## Sammenhængen mellem minutvolumen, slagvolumen og puls

Hjertets primære opgave er at pumpe tilstrækkeligt iltrigt blod rundt til kroppens organer og arbejdende muskler. Hjertets samlede pumpekapacitet styres af tre forbundne faktorer:

$$\text{Minutvolumen (MV)} = \text{Slagvolumen (SV)} \times \text{Pulsfrekvens (HR)}$$

1. **Minutvolumen (MV):** Den samlede mængde blod, hjertet pumper ud pr. minut (måles i L/min).
2. **Slagvolumen (SV):** Mængden af blod, hjertet pumper ud for **hvert enkelt hjerteslag** (måles i mL/slag).
3. **Pulsfrekvens (HR):** Antallet af hjerteslag pr. minut (måles i BPM).

### 🎛️ Interaktiv simulator: Udforsk hjertekredsløbet

Prøv simulatoren herunder for at se, hvordan ændringer i henholdsvis din puls og dit slagvolumen direkte påvirker hjertets samlede minutvolumen:

{% include components/puls-interaktiv.html %}

### Hvordan påvirker træning denne sammenhæng?

I hvile har din krop brug for en konstant minutvolumen på ca. **5 liter blod pr. minut** for at holde stofskiftet i gang:

* **Hos en utrænet person:** Hjertets slagvolumen i hvile er relativt lille (fx $70 \text{ mL/slag}$). For at nå $5 \text{ L/min}$ skal hjertet slå ca. **71 gange i minuttet** ($0{,}070 \text{ L} \times 71 \text{ BPM} \approx 5{,}0 \text{ L/min}$).
* **Hos en veltrænede atlet:** Konditionstræning gør hjertets venstre hjertekammer større og stærkere, så slagvolumen i hvile stiger markant (fx til $120 \text{ mL/slag}$). Hjertet behøver nu kun slå **41 gange i minuttet** for at pumpe den samme mængde blod rundt ($0{,}120 \text{ L} \times 41 \text{ BPM} \approx 5{,}0 \text{ L/min}$). Det er grunden til, at veltrænede får en meget lav hvilepuls!

Under maksimalt arbejde kan en eliteatlet øge sin minutvolumen helt op til **35–40 L/min**, primært drevet af et enormt slagvolumen, da maxpulsen falder med alderen uanset form.
...
{% include figure image_path="/assets/images/hjerteforeningen.dk/hjertet500pxbred-c0d20495.jpg" caption="Kilde: [Hjerteforeningen](https://hjerteforeningen.dk/hjertet-og-kredsloebet/). [📷](https://hjerteforeningen.dk/wp-content/uploads/2019/01/hjertet500pxbred.jpg){: rel='nofollow noopener' }" %}
...
De fleste mennesker har en hvilepuls på 60 til 100 slag i minuttet. Men det er forskelligt for forskellige personer og på forskellige tidspunkter for den samme person. Din hvilepuls kan fx være lavere, når du er veludhvilet, eller hvis du tager visse former for medicin.
...
Du kan [beregne din pulsreserve](/pulszoner-beregner/) i vores beregner, hvis du gerne vil arbejde med begrebet.
...
En lav HRV betyder, at det sympatiske og parasympatiske nervesystem ikke arbejder godt sammen, hvilket kan føre til uregelmæssig hjerterytme.
...
{% include figure image_path="/assets/images/polar.com/The-new-Polar-H10-heart-rate-sensor-is-now-available_feature-bb9ce8de.jpg" caption="Det mest præcise er at måle puls med en brystrem, når du bruger et pulsur. Kilde: [Polar](https://www.polar.com/blog/new-polar-h10-heart-rate-sensor-2017/) [📷](https://www.polar.com/blog/wp-content/uploads/2017/03/The-new-Polar-H10-heart-rate-sensor-is-now-available_feature.jpg){: rel='nofollow noopener' }" alt="mål puls med pulsur" %}

### 3. Mål pulsen med blodtryksmåler
...
Hvis du gerne vil i gang med pulstræning, så tjek vores guide til [pulszoner og pulsreserve](/pulszoner-beregner/).
...
Pulsen bliver ofte brugt i forskellige [konditionstests](/kondition/tests/). Mange er interesseret i at [finde dit kondital](/kondital/). Det gør den, fordi pulsen kan være med til at sige noget om den intensitet, du arbejder med på en bestemt belastning.
...
{% include figure image_path="https://images.unsplash.com/photo-1523394643039-a2770cf4a2a0?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxzZWFyY2h8M3x8aGVhcnQlMjByYXRlfGVufDB8fDB8fA%3D%3D&auto=format&fit=crop&w=600&q=60" caption="Et pulsur kan være en god investering, hvis du gerne vil træne pulstræning, men kun hvis du sætter dig ind i, hvad puls og makspuls er." %}
...
Det er vigtigt, hvis du køber et pulsur, at du investerer lidt af din tid og vilje i at få lavet en ordentlig test af din maksimale hjertefrekvens, som jeg fortalte om lidt tidligere i artiklen.
