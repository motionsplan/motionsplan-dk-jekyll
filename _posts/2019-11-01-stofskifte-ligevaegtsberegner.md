---
title: Beregn dit ligevægtsindtag og daglige energibehov ⚖️
seo_title: "Ligevægtsberegner: Beregn dit ligevægtsindtag nemt & gratis"
description: "Beregn dit præcise ligevægtsindtag (TDEE). Indtast din højde, vægt og aktivitet, og find ud af præcis hvor mange kalorier du skal spise om dagen!"
permalink: /ligevaegtsindtag-beregner/
classes: wide
redirect_from:
  - /ligevaegtsindtag/
  - /beregner/ligevaegtsindtag-med-ligevaegtsberegner/
  - /beregner/dagligt-stofskifte-og-forbraending/
  - /avanceret-stofskifteberegner/
  - /beregner/dagligt-energiforbrug-energibehov/
  - /beregner/stofskifte-energiforbrug-energibehov/
language: da
header:
  teaser: /assets/images/unsplash/photo-1582004228576-14e8b1256618.jpg
  credit: https://images.unsplash.com/photo-1582004228576-14e8b1256618
  caption: Beregn dit ligevægtsindtag og daglige energibehov ⚖️
category:
  - Kost & Ernæring
  - Kalorieberegnere
tags:
  - beregner
  - stofskifte
  - ligevægtsindtag
  - energiberegner
  - tdee
  - pal
meta:
  name: "Ligevægtsindtag og TDEE"
  measures: "ligevægtsindtag, energiforbrug, TDEE, PAL, BMR"
  type: "beregner"
  equipment: "ingen"
  intensity: "ingen"
  method: "formel"
tools:
  - id: "tool-ligevaegtsindtag-tdee-beregner"
    title: "Ligevægtsindtag & TDEE Beregner"
    description: "Beregn dit daglige ligevægtsindtag (TDEE) ud fra dit hvilestofskifte (BMR) og dit fysiske aktivitetsniveau (PAL) med enten hurtig PAL-faktor eller 24-timers tidsregistrering."
    category: ["Ernæring"]
    type: ["Beregner"]
    measures: ["Ligevægtsindtag", "TDEE", "Energibehov", "Energiforbrug"]  # 🎯 Output-værdier fra TDEE-modulet
    anchor: "#calculator"
    category_schema: "HealthAndFitnessApplication"

  - id: "tool-vaegtaendring-kalorie-beregner"
    title: "Vægtændring & Kalorieberegner"
    description: "Beregn din potentielle vægtændring over tid baseret på et specifikt kalorieunderskud (cut) eller kalorieoverskud (bulk)."
    category: ["Ernæring"]
    type: ["Beregner"]
    measures: ["Vægtændring", "Vægttab", "Kalorieunderskud", "Kalorieoverskud"]  # 🎯 Output-værdier fra vægtændringsmodulet
    anchor: "#beregn-din-potentielle-vaegtaendring-over-tid"
    category_schema: "HealthAndFitnessApplication"
popular: 'true'
last_modified_at: '2026-07-24T00:00:00Z'
toc: 'true'
keywords:
  - ligevægtsindtag
  - beregn ligevægtsindtag
  - ligevægtsindtag beregner
  - beregn energiforbrug
  - dagligt energibehov
  - tdee beregner
  - pal faktor
faq:
- question: Hvad er mit ligevægtsindtag (TDEE)?
  answer: Ligevægtsindtaget (Total Daily Energy Expenditure / TDEE) er den mængde energi, du skal indtage gennem kosten i løbet af en dag for at holde din vægt fuldstændig stabil.
- question: Hvordan beregnes det daglige energibehov?
  answer: Dit daglige energibehov beregnes ved at gange dit hvilestofskifte (BMR) med din samlede aktivitetsfaktor (PAL), som dækker over din daglige fysiske aktivitet.
- question: Hvad er forskellen på BMR og ligevægtsindtag?
  answer: BMR (hvilestofskiftet) er den energi, kroppen bruger i fuldstændig hvile blot for at holde livsprocesserne i gang. Ligevægtsindtaget inkluderer BMR plus energiforbruget fra al din daglige bevægelse og træning.
- question: Hvad er PAL (Physical Activity Level)?
  answer: PAL er en faktor, der beskriver dit samlede fysiske aktivitetsniveau i løbet af et døgn. Formlen er TDEE = BMR × PAL.
- question: Er en online ligevægtsberegner helt præcis?
  answer: Beregneren giver et fagligt stærkt estimat baseret på gennemsnitlige formler. For at finde dit helt nøjagtige ligevægtsindtag anbefales det at kombinere beregningen med 14 dages kostregistrering og daglig vejning.
- question: Hvad skal jeg gøre, hvis jeg vil tabe mig eller opbygge muskelmasse?
  answer: For at tabe fedt skal du indtage færre kalorier end dit ligevægtsindtag (kalorieunderskud / cut). For at optimere muskelopbygning skal du indtage flere kalorier end dit ligevægtsindtag (kalorieoverskud / bulk).
- question: Hvordan finder jeg min samlede daglige forbrænding?
  answer: Din samlede daglige forbrænding (TEE/TDEE) findes ved at gange dit hvilestofskifte (BMR) med din aktivitetsfaktor (PAL). Du kan beregne den præcist i vores beregner her på siden.
- question: Hvad er forskellen på TEE, TDEE og ligevægtsindtag?
  answer: TEE (Total Energy Expenditure) og TDEE (Total Daily Energy Expenditure) er de videnskabelige betegnelser for dit samlede daglige energiforbrug. Ligevægtsindtag er det antal kalorier, du skal spise for at matche dette energiforbrug præcist.
- question: Kan jeg beregne min forbrænding nøjagtigt, hvis jeg ønsker at tabe mig?
  answer: Ja, ved at kende din daglige forbrænding og dit ligevægtsindtag kan du oprette et præcist kalorieunderskud (fx 500 kcal om dagen) for at opnå et jævnt og sundt vægttab.
- question: Er en beregner til ligevægtsindtag præcis?
  answer: 'En online beregner giver et kvalificeret skøn baseret på gennemsnitlige værdier for basalstofskifte og fysisk aktivitet. Den kan være utrolig præcis for mange, men individuelle faktorer som kropssammensætning, genetik, søvn og stress kan skabe variationer.'
feature_row_needed:
- title: ⚖️
  excerpt: '**Køkkenvægt**<br>For at registrere dit faktiske kalorieindtag skal du veje din mad. Uden en køkkenvægt risikerer du markant at under- eller overvurdere dit kalorieindtag.'
- title: 📏
  excerpt: '**Badevægt**<br>Med en badevægt kan du følge din vægtudvikling over tid. Vægtændringer over 14 dage viser, om du reelt rammer dit ligevægtsindtag.'
- title: 📲
  excerpt: '**Kalorie-app**<br>Brug en app til nemt at indtaste din vejede mad. [Se vores overblik over kalorietæller-apps](/kalorietaeller-apps/).'
templates:
  - id: "pal_aktivitetsregistrering_skema"
    title: "Skema til 24-timers aktivitetsregistrering"
    description: "Du kan udskrive eller kopiere dette skema og udfylde det for en typisk dag. Når du har udregnet timerne i de enkelte kategorier, kan du indtaste værdierne i beregneren."
    format: "Google Docs"
    type: "doc"
    btn_text: "📝 Åbn registreringsskema"
    url: "https://docs.google.com/document/d/1pWnzxIkn3h_deY7pavw-OnCxhvtSiyFfBZCoyJjj8dY/copy?usp=sharing"
    image: "/assets/images/pal-schedule.png"
---

Vil du kende dit præcise **ligevægtsindtag** – altså det antal kalorier, du skal spise om dagen for at holde vægten stabil, tabe dig eller opbygge muskelmasse?

Det samlede daglige energiforbrug betegnes ofte som **TDEE** (*Total Daily Energy Expenditure*). Dit ligevægtsindtag findes ved at kombinere dit [hvilestofskifte (BMR)](/bmr-beregner/) med dit samlede fysiske aktivitetsniveau (**[PAL-faktor](/beregner/pal/)**).

I denne beregner kan du vælge mellem to metoder:
1. **Den hurtige metode:** Vælg et generelt aktivitetsniveau fra en PAL-tabel.
2. **Den normal metode:** Indtast din tidsfordeling over døgnets 24 timer i forskellige kategorier for et mere nøjagtigt resultat.
2. **Den avancerede metode:** Indtast din tidsfordeling for alle døgnets 24 timer på specifikke aktiviteter for et mere nøjagtigt resultat.

## Beregn dit ligevægtsindtag (TDEE) og energibehov
{: id="calculator" }

Vælg den formel for hvilestofskiftet, der passer bedst til dig (beregneren vælger automatisk den anbefalede ud fra alder og køn). Derefter angiver du dit aktivitetsniveau enten via drop-down eller via præcis tidsregistrering.

{% include calc/bmr-calculator.html 
  title="Beregn dit ligevægtsindtag (TDEE)"
  description="Beregn dit samlede energibehov på en dag ud fra hvilestofskiftet (BMR) og dit daglige aktivitetsniveau (PAL)."
%}

{: .notice--info}
**Kender du din fedtprocent?**  
Hvis du er meget veltrænet eller har en høj muskelmasse, kan du benytte vores [BMR-beregner ud fra fedtfri masse (FFM)](/bmr-beregner-ffm/).

Med denne beregner kan du udregne dit samlede daglige energibehov, dit daglige stofskifte og din totale forbrænding (TEE / TDEE). Resultatet giver et præcist estimat baseret på gennemsnitsværdier for dit køn, din alder, din vægt og dit fysiske aktivitetsniveau.


*[BMR]: Basic Metabolic Rate
*[REE]: Resting Energy Expenditure
*[PAL]: Physical Activity Level
*[FIT]: Fødeinduceret termogenese
*[TEE]: Total Energy Expenditure
*[TDEE]: Total Daily Energy Expenditure
*[kJ]: kilojoule
*[MET]: Metabolic Equivalent of Task

---

## Stofskiftets 3 komponenter

Dine celler forbrænder energi døgnet rundt. Det samlede daglige energiforbrug består af tre primære komponenter:

$$
\text{Samlet Energiforbrug (TDEE)} = \text{BMR} + \text{FIT} + \text{Aktivitet (PAL)}
$$

* **Hvilestofskiftet (BMR/REE):** Den energi kroppen bruger på grundlæggende livsprocesser i hvile (udgør ca. 60–75% af det samlede forbrug).
* **Fødeinduceret termogenese (FIT):** Den energi kroppen bruger på at fordøje og optage næringsstoffer fra maden (udgør ca. 10%).
* **Fysisk aktivitet (PAL):** Energiforbruget fra al bevægelse – både struktureret træning, arbejde og hverdagsbevægelse.

Du kan læse mere om [fysiologien bag stofskiftet her](/stofskifte/).

---

## Hvilken formel skal jeg vælge til beregning af ligevægtsindtag og stofskifte?

Der findes en lang række forskellige formler til udregning af hvilestofskifte, basalstofskifte og BMR. Hvis du vælger **Anbefalet formel** i beregneren, vælger systemet automatisk den mest passende formel ud fra din alder og dit køn.

Formlerne tager højde for forskellige aldersgrupper og befolkningstyper, da energiforbruget i hvile ændrer sig med kropsvægt, højde og alder. Hvis du vil nørde fysiologien og matematikken bag de enkelte modeller, kan du læse vores dybdegående artikel om [formler for hvilestofskiftet](/hvilestofskiftet-formler/).

---

## Metode 1: Hurtig beregning med PAL-tabellen

Den mest almindelige måde at beregne ligevægtsindtaget på er ved at gange BMR med en fast **PAL-faktor** (*Physical Activity Level*). 

Tabellen herunder stammer fra *Nordic Nutrition Recommendations (2012)*:

| Aktivitetsniveau | PAL-værdi | Beskrivelse |
| :--- | :---: | :--- |
| **Sengeliggende / Inaktiv** | 1,2 | Rullestolsbunden eller sengeliggende. |
| **Stillesiddende arbejde** | 1,4–1,5 | Stillesiddende arbejde med lidt eller ingen aktivitet i fritiden. |
| **Stillesiddende med let aktivitet** | 1,6–1,7 | Stillesiddende arbejde med behov for gang eller let træning i fritiden. |
| **Stående arbejde** | 1,8–1,9 | Hovedsageligt stående eller gående arbejde (fx butik, undervisning). |
| **Hård træning / Hårdt kropsarbejde** | 2,0–2,4 | Hårdt fysisk arbejde eller meget høj træningsmængde dagligt. |

*Bemærk: Hvis du har stillesiddende arbejde, men træner hårdt 30–60 minutter 4–5 gange om ugen, lægges der typisk ca. +0,3 til din PAL-faktor.*

> **Eksempel:**  
> En mand har et beregnet BMR på 7.600 kJ (ca. 1.815 kcal). Han har stillesiddende kontorarbejde (PAL = 1,4), men træner i motionscenter 4 gange om ugen (+0,3). Hans samlede PAL er 1,7.  
> **Ligevægtsindtag:** $7.600 \text{ kJ} \times 1,7 = \mathbf{12.920 \text{ kJ/døgn}}$ (ca. 3.088 kcal).

Du kan dykke dybere ned i [hvordan PAL og MET-værdier udregnes i vores specialartikel om PAL](/beregner/pal/).

---

## Metode 2: Præcis tidsregistrering (24-timers skema)

Hvis din dag er meget sammensat, kan det give en mere præcis beregning at opgøre døgnets 24 timer i specifikke aktivitetskategorier.

Udregningen bygger på [Gerrior et al. (2006)](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1784117/), som anvender estimerede MET-værdier for døgnets aktiviteter:

| Aktivitet | MET-værdi | Eksempler |
| :--- | :---: | :--- |
| **Sove** | MET 0,9 | Tid i sengen i dyb hvile eller søvn. |
| **Siddende** | MET 1,2 | Kontorarbejde, lektielæsning, TV, spise, køre bil. |
| **Stående og gående** | MET 2,0 | Madlavning, let gang, indkøb, opvask. |
| **Let aktivitet** | MET 4,0 | Rengøring, havearbejde, rask gang, golf. |
| **Moderat træning** | MET 7,0 | Styrketræning, cykling i moderat tempo, svømning, boldspil. |
| **Intens træning** | MET 10,0 | Hård løbetræning, hockeyspurt, intensiv roning, HIIT. |

For at gøre tidsregistreringen nem kan du hente vores skema:

{% include motionsplan/templates.html id="pal_aktivitetsregistrering_skema" %}

I beregneren indtaster du det samlede antal timer for hver kategori, så summen giver 24 timer. Beregneren vægter derefter aktiviteterne mod dine MET-værdier for at finde din nøjagtige daglige PAL-faktor.

---

## Sådan finder du dit helt præcise ligevægtsindtag i praksis

En online beregner giver et teoretisk estimat. Fordi der er individuelle forskelle i muskelmasse, tarmflora, hormoner og hverdagsbevægelse (NEAT), bør du bruge beregnerens resultat som et **startpunkt**.

For at finde dit empirisk 100% præcise ligevægtsindtag skal du gennemføre en **14-dages test**:

<div class="feature__wrapper-emoji">
{% include feature_row id="feature_row_needed" %}
</div>

### Step-by-step guide til 14-dages testen:
1. **Uge 1–2 (Præcis vejning af mad):** Vej al din mad på en køkkenvægt og tast det i en kalorie-app. Spis det antal kalorier, som beregneren ovenfor har anslået.
2. **Daglig vejning:** Vej dig hver morgen efter toiletbesøg og før morgenmad. Skriv vægten ned.
3. **Evaluer gennemsnittet:** Sammenlign dit gennemsnitlige vægttal fra uge 1 med uge 2:
   * **Stabil vægt:** Tillykke, du har fundet dit eksakte ligevægtsindtag!
   * **Vægten stiger:** Du er i kalorieoverskud. Sænk dit daglige indtag med 150–300 kcal.
   * **Vægten falder:** Du er i kalorieunderskud. Øg dit daglige indtag med 150–300 kcal.

Ønsker du den mest **nøjagtige TEE-beregning**, kan du enten benytte vores [tidsregistrering med MET-skema](#metode-2-praecis-tidsregistrering-24-timers-skema) eller følge vores 14-dages test med kostregistrering og badevægt.

---

## Hvad kan jeg bruge ligevægtsindtaget og energiforbruget til?

Når du kender dit ligevægtsindtag og din daglige forbrænding, har du det ideelle fundament til at sammensætte din kost og træning:

- **Hold vægten stabil:** Indtag nøjagtigt det antal kalorier, beregneren anslår.
- **Vægttab (Cut):** Læg dig 300–500 kcal under dit ligevægtsindtag for et kontrolleret fedttab uden tab af muskelmasse.
- **Muskelopbygning (Bulk):** Læg dig 200–400 kcal over dit ligevægtsindtag for at give kroppen de nødvendige byggesten til at opbygge ny muskelmasse.

---

## Brug ligevægtsberegneren igen, når din vægt ændrer sig

Din samlede daglige forbrænding afhænger direkte af din kropsvægt. 

- Hvis du **taber dig**, falder dit hvilestofskifte og dit ligevægtsindtag automatisk, fordi kroppen har mindre væv at vedligeholde og flytte på.
- Hvis du **tager på i vægt** (især muskelmasse), vil dit daglige energibehov stige.

Husk derfor altid at foretage en ny beregning af dit ligevægtsindtag, når din kropsvægt har ændret sig markant.

---

## Vægtændring: Cut, Bulk og Kaloriejustering

Når du kender dit ligevægtsindtag, kan du målrette din kost i forhold til dine kropslige mål:

* **Cut (Kalorieunderskud):** Du indtager færre kalorier end dit ligevægtsindtag (typisk 300–500 kcal under) for at forbrænde kropsfedt.
* **Bulk (Kalorieoverskud):** Du indtager flere kalorier end dit ligevægtsindtag (typisk 200–400 kcal over) for at give kroppen energi til maksimal muskelopbygning.

### Beregn din potentielle vægtændring over tid

Brug beregneren herunder til at se, hvordan et specifikt kalorieoverskud eller -underskud vil påvirke din vægt over uger og måneder:

{% include calculator/calculator-weightloss.html %}

---

## ✅ Næste skridt i din kostplanlægning

Når du har styr på dit samlede kaloriemål, er næste naturlige skridt at kigge på, hvordan kalorierne skal fordeles på makronæringsstoffer (protein, fedt og kulhydrat):

- 🥗 **Fordel dine kalorier:** Brug vores [Energifordeling- og Makro-beregner](/beregner/energifordeling/)
- 📖 **Lær om energitætheden i mad:** Læs om [hvad kalorier egentlig er](/hvad-er-kalorier/)
- 🔍 **Nørder du formler?** Se vores dybdegående gennemgang af [formlerne for hvilestofskiftet](/hvilestofskiftet-formler/)

---

## Ofte stillede spørgsmål om ligevægtsindtag

{% include motionsplan/faq.html %}

<details markdown="1" class="references">
  <summary><h2 id="references">Referencer</h2></summary>

- Gerrior, Shirley, WenYen Juan, og Basiotis Peter. 2006. “An Easy Approach to Calculating Estimated Energy Requirements”. *Preventing Chronic Disease* 3 (4). <https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1784117/>.
- Nordic Council of Ministers. 2014. *Nordic Nutrition Recommendations 2012: Integrating Nutrition and Physical Activity*. 5th ed. Copenhagen. <https://www.norden.org/en/publication/nordic-nutrition-recommendations-2012>.
- Schofield, W. N. 1985. “Predicting Basal Metabolic Rate, New Standards and Review of Previous Work”. *Human Nutrition. Clinical Nutrition* 39 Suppl 1: 5–41.
- Waterlow, John C., Nevin S. Scrimshaw, og Beat Schürch. 1996. “Energy and Protein Requirements, Proceedings of an IDECG Workshop”. *European Journal of Clinical Nutrition* 50 (Feb): 1–197.
- Westerterp, Klaas R. 2013. “Physical Activity and Physical Activity Induced Energy Expenditure in Humans: Measurement, Determinants, and Effects”. *Frontiers in Physiology* 4. <https://doi.org/10.3389/fphys.2013.00090>.
</details>