---
title: 'Taljemål, Talje-Højde & Talje-Hofte Ratio: Mål din sundhed'
seo_title: 'Taljemål & Ratios: Beregn din livvidde, WHR og talje-højde-forhold'
description: 'Beregn din talje-hofte-ratio (WHR) og talje-højde-ratio. Indtast dit taljemål, få en præcis vurdering ud fra videnskabelige grænseværdier og skemaer.'
excerpt: 'Dit taljemål og dets forhold til din højde og hofte siger meget om din sundhed og risikoprofil. Brug vores samlede beregner-dashboard og følg de grundige måle-guides.'
permalink: /taljemaal/
redirect_from:
  - /talje-hoejde-beregner/
  - /talje-hofte-ratio/
language: da
header:
  teaser: /assets/images/unsplash/photo-1523901839036-a3030662f220.jpg
  credit: https://images.unsplash.com/photo-1523901839036-a3030662f220
  caption: Taljemål, livvidde, hoftemål, højdemål og talje-hofte-ratioen.
categories:
  - Antropometri
tags:
  - måling
  - test
  - kropskomposition
  - tracking
  - featured
  - antropometri
  - beregner
popular: 'true'
last_modified_at: '2026-07-24T00:00:00Z'
toc: 'true'
faq:
  - question: Hvad er et normalt taljemål for mænd og kvinder?
    answer: For kvinder anbefales et taljemål under 80 cm (forøget risiko ved > 88 cm). For mænd anbefales et taljemål under 94 cm (forøget risiko ved > 102 cm).
  - question: Hvad er et normalt hoftemål og talje-hofte-ratio (WHR)?
    answer: Det er svært at angive et ideelt hoftemål i cm, men i forholdet mellem talje og hofte bør kvinder helst ligge under 0,8, og mænd bør helst ligge under 1,0.
  - question: Hvor og hvordan måler man taljen og hoften?
    answer: Taljen måles ca. 2 cm over navlen ved det smalleste punkt mellem hofte og nederste ribben. Hoften måles stående fra siden på det bredeste sted over sædemusklerne.
  - question: Hvad er talje-højde-ratioen?
    answer: Talje-højde-ratioen sætter dit taljemål i forhold til din højde. Tommelfingerreglen er, at dit taljemål bør være under halvdelen af din højde (ratio under 0,5).
meta:
  name: "Taljemål og Ratios"
  measures: "taljemål, talje-hofte-ratio, talje-højde-ratio"
  type: "antropometri"
  equipment: "maalebaand"
  intensity: "ingen"
  method: "direkte"
page_type: spoke
# TESTS / PROTOKOLLER
tests:
  - id: "test-taljemaal-livvidde"
    title: "Måling af taljemål (Livvidde)"
    description: "Direkte måling af taljeomkreds 2 cm over navlen med målebånd til vurdering af mavefedt og sundhedsrisiko."
    category: ["Antropometri", "Kropskomposition"]
    type: ["Protokol", "Måling"]
    execution: ["Måling"]
    method: "direkte"                  # 📏 Direkte fysisk måling i cm
    modality: ["Kropsmål"]
    measures: ["Taljemål", "Livvidde"]
    equipment: ["Målebånd"]
    setting: ["Individuel"]
    target_group: ["Voksne", "Mænd", "Kvinder"]
    related_tools: ["tool-samlet-taljemaal-dashboard"]

  - id: "test-whtr"
    title: "Talje-Højde-Ratio (WHtR)"
    description: "Måling der sætter taljemål i forhold til kropshøjde. Bruges i Ashwell Shape Chart til at vurdere sundhedszoner."
    category: ["Antropometri", "Kropskomposition"]
    type: ["Protokol", "Måling"]
    execution: ["Måling"]
    method: "direkte"                  # 📏 Baseret på direkte kropsmål (talje + højde)
    modality: ["Kropsmål"]
    measures: ["Talje-højde-ratio", "WHtR"]
    equipment: ["Målebånd", "Højdemåler"]
    setting: ["Individuel"]
    target_group: ["Børn", "Unge", "Voksne", "Ældre", "Mænd", "Kvinder"]
    related_tools: ["tool-samlet-taljemaal-dashboard", "tool-whtr-beregner"]

  - id: "test-whr"
    title: "Talje-Hofte-Ratio (WHR)"
    description: "Måling der vurderer fedtfordeling (æbleform vs. pæreform) baseret på WHO's grænseværdier for talje- og hoftemål."
    category: ["Antropometri", "Kropskomposition"]
    type: ["Protokol", "Måling"]
    execution: ["Måling"]
    method: "direkte"                  # 📏 Baseret på direkte kropsmål (talje + hofte)
    modality: ["Kropsmål"]
    measures: ["Talje-hofte-ratio", "WHR", "Kropsform"]
    equipment: ["Målebånd"]
    setting: ["Individuel"]
    target_group: ["Voksne", "Mænd", "Kvinder"]
    related_tools: ["tool-samlet-taljemaal-dashboard", "tool-whr-beregner"]

# INTERAKTIVE SOFTWARE-VÆRKTØJER
tools:
  - id: "tool-samlet-taljemaal-dashboard"
    title: "Samlet Taljemål & Ratio Beregner"
    description: "Beregn dit taljemål, din talje-højde-ratio (WHtR) og din talje-hofte-ratio (WHR) samlet i ét dashboard med øjeblikkelig sundhedsanalyse."
    category: ["Antropometri"]
    type: ["Beregner"]
    execution: ["Testberegner"]
    anchor: "#beregn-alle-dine-taljemaal"
    category_schema: "HealthAndFitnessApplication"

  - id: "tool-whtr-beregner"
    title: "Talje-Højde-Ratio Beregner (WHtR)"
    description: "Beregn dit talje-højde-forhold og se din placering i Ashwell Shape Chart over sundhedszoner."
    category: ["Antropometri"]
    type: ["Beregner"]
    execution: ["Måling"]
    anchor: "#2-talje-højde-ratio-whtr"
    category_schema: "HealthAndFitnessApplication"

  - id: "tool-whr-beregner"
    title: "Talje-Hofte-Ratio Beregner (WHR)"
    description: "Beregn din talje-hofte-ratio og få vurderet din fedtfordeling (æbleform vs. pæreform) ud fra WHO's grænseværdier."
    category: ["Antropometri"]
    type: ["Beregner"]
    execution: ["Måling"]
    anchor: "#3-talje-hofte-ratio-whr"
    category_schema: "HealthAndFitnessApplication"
---

Dit taljemål er en enkel og utrolig præcis måling, der siger meget om din sundhed. Studier viser, at fedtet omkring maveområdet (det intraabdominale fedt) er direkte knyttet til risikofaktorer for livsstilssygdomme som hjertekarsygdomme og type 2-diabetes.

På denne side kan du beregne alle de tre mest anerkendte antropometriske taljemål på ét samlet sted:
1. **[Det rene taljemål (livvidde)](#1-det-rene-taljemål-livvidde)** – Betydningen af dit mavefedt i centimeter.
2. **[Talje-Højde-Ratio](#2-talje-højde-ratio-whtr)** – Vurderer dit taljemål i forhold til din kropshøjde.
3. **[Talje-Hofte-Ratio (WHR)](#3-talje-hofte-ratio-whr)** – Afspejler fedtfordelingen på kroppen (æble- vs. pæreform).

*[WHR]: Waist/Hip-ratio

---

## Beregn alle dine taljemål

Indtast din højde, dit taljemål og dit hoftemål herunder for at få en komplet analyse af alle tre parametre på én gang.

{% include calc/waist-dashboard.html %}

---

## Guider: Sådan måler du dine mål korrekt

For at få pålidelige og reproducerbare resultater er det afgørende, at du måler de præcise steder på kroppen med et fleksibelt målebånd.

### 1. Sådan måler du dit taljemål (livvidde)

* **Position:** Stå afslappet, rank og udånd naturligt (undgå at suge maven ind).
* **Placering:** Sæt målebåndet vandret omkring 2 cm over navlehøjden – på det punkt hvor taljen naturligt er smallest (mellem øverste hoftebenskam og nederste ribben).

{% include video provider="youtube" id="Cytz_9lMl68" %}

---

### 2. Sådan måler du dit hoftemål

* **Position:** Stå oprejst med samlede ben.
* **Placering:** Før målebåndet omkring det bredeste sted over hofterne og sædemusklerne (ballerne). Det er nemmest at se det korrekte punkt ved at kigge på kroppen fra siden i et spejl.

{% include figure image_path="/assets/images/myfooddiary.com/waist_to_hip_measure-c38b9e96.gif" caption="Kilde: myfooddiary.com" alt="Måling af talje og hoftemål" %}

{% include video provider="youtube" id="gUJXbTl4qzg" %}

---

## 1. Det rene taljemål (Livvidde)

Taljemålet er i sig selv særligt relevant, fordi det måler det risikable fedt omkring de indre organer. I forbindelse med vægttab og styrketræning er taljemålet ofte en langt bedre markør end vægten eller [BMI-beregninger](/bmi/), da øget muskelmasse kan øge kropsvægten, selvom du taber fedt.

{% include calc/waist.html 
   calc_id="waist-only" 
   title="📐 Vurdering af taljemål" 
%}

### Grænseværdier og sundhedsrisiko

Nedenfor ses de officielle grænseværdier ifølge bl.a. [Hjerteforeningen](https://hjerteforeningen.dk/2013/05/fedt-paa-maven-er-ufedt/):

| Køn | Normal / Lav risiko | Let forøget risiko | Forøget risiko |
| :--- | :--- | :--- | :--- |
| **Mænd** | < 94 cm | > 94 cm | > 102 cm |
| **Kvinder** | < 80 cm | > 80 cm | > 88 cm |

Da en opdeling i blot tre kategorier er forenklet, har forskeren **Bray (2004)** foreslået en mere finmasket inddeling af livvidden:

| Kategori | Kvinder | Mænd |
| :--- | :--- | :--- |
| **Meget lav** | < 70 cm | < 80 cm |
| **Lav** | 70–89 cm | 80–99 cm |
| **Høj** | 90–109 cm | 100–120 cm |
| **Meget høj** | > 110 cm | > 120 cm |

---

## 2. Talje-Højde-Ratio (WHtR)

En klar ulempe ved det rene taljemål er, at det ikke tager højde for personens højde. En meget høj person vil naturligt have en større taljeomkreds end en lav person. 

Derfor vinder **talje-højde-ratioen** stadigt større indpas i videnskabelig litteratur. Den kan anvendes på både børn og voksne uanset kropsbygning.

{% include calc/waist.html 
   calc_id="waist-height" 
   title="📐 Beregn Talje-Højde-Ratio" 
%}

### Hvorfor er WHtR bedre end Sundhedsstyrelsens taljemål?

Sundhedsstyrelsen opererer med faste grænseværdier (fx over 80 cm for kvinder og 94 cm for mænd), men de tal tager ikke højde for, hvor høj du er. Det skaber et åbenlyst problem:

* En kvinde på **155 cm** med et taljemål på 78 cm har relativt meget mavefedt og er i risikozonen (Hendes ratio er 0,50). Alligevel "godkendes" hun af Sundhedsstyrelsen.
* En kvinde på **185 cm** med et taljemål på 82 cm er slank og har en uproblematisk fedtfordeling (Hendes ratio er 0,44). Alligevel placeres hun i farezonen hos Sundhedsstyrelsen.

**Talje-højde-ratioen (WHtR)** fjerner denne skævvridning. Forskning har gentagne gange vist, at netop dette forholdstal er langt mere præcist til at forudsige risikoen for hjertekarsygdomme og type 2-diabetes end både BMI og almindeligt taljemål.

### Tommelfingerregel & Ashwell Shape Chart®

En god grundregel er: **Dit taljemål bør være mindre end halvdelen af din højde (Ratio < 0,5)**.

| Ratio (WHtR) | Klassifikation | Betydning |
| :--- | :--- | :--- |
| **Under 0,4** | Måske for tynd | Vær opmærksom på undervægt |
| **0,4 til 0,5** | Sund pæreform | Optimal og sund profil |
| **0,5 til 0,6** | Mellem pære og æble | Overvej livsstilsændringer |
| **Over 0,6** | Usund æbleform | Forhøjet risiko – handling anbefales |

Klassifikationen bygger blandt andet på *Ashwell Shape Chart®* (Ashwell et al., 2011/2014), som overskueligt opdeler kropsformer i sundhedszoner.

{% include figure image_path="https://www.ashwell.uk.com/images/shapechart.jpg" caption="Ashwell Shape Chart® viser grafisk risikozoner baseret på talje-højde-forholdet." alt="Ashwell Shape Chart for talje-højde ratio" %}

---

## 3. Talje-Hofte-Ratio (WHR)

Hvor talje-højde-ratioen kigger på fedtmængde i forhold til højden, siger **talje-hofte-ratioen (WHR)** noget om **fedtets fordeling** på kroppen.

Forholdet afspejler, om man har tendens til en **æbleformet kropsbygning** (fedtet sidder overvejende på maven) eller en **pæreformet kropsbygning** (fedtet sidder primært på hofter og lår). Det intraabdominale mavefedt ved æbleform er sundhedsmæssigt mere risikabelt end fedtdepoter på hofterne.

{% include calc/waist.html 
   calc_id="waist-hip" 
   title="📐 Beregn Talje-Hofte-Ratio" 
%}

{% include figure image_path="/assets/images/blog/Body-shapes.jpg" caption="Pæreform (fedt på hofter) vs. æbleform (fedt om maven)." alt="Pæreform og æbleform kropsfaconer" %}

### Normer for WHR (WHO)

Ifølge Verdenssundhedsorganisationen (WHO) og international litteratur er risikogrænserne for WHR opstillet således:

| Køn | Sund fedtfordeling | Øget risiko (Æbleform) |
| :--- | :--- | :--- |
| **Mænd** | $\le 1,0$ | > 1,0 |
| **Kvinder** | $\le 0,8$ | > 0,8 |

---

## Sammenligning & Konklusion

De tre antropometriske mål supplerer hinanden ideelt:
* **Det rene taljemål** er hurtigt og nemt at bruge i hverdagen.
* **Talje-højde-ratioen** giver det mest retfærdige billede, hvis du er meget høj eller meget lav.
* **Talje-hofte-ratioen** afslører din kropsfacon og fedtdistribution.

*(Bemærk også, at der findes målingsmetoder som [Body Adiposity Index (BAI)](/bai-body-adiposity-index/), som beregner fedtprocent udelukkende ud fra hoftemål og højde).*

> 📌 **Husk:** En online test eller beregning kan give et godt fingerpeg om din sundhedsprofil, men bør aldrig stå alene eller være startskud til en ekstrem slankekur.

## Vil du vide mere om din kropsfedtprocent?

Taljemålet og dine ratios giver et godt billede af din sundhed og fedtfordeling, men måler ikke din samlede fedtmasse i kilo eller procent. 

Hvis du vil dykke dybere ned i din kropskomposition, kan du læse mere i vores andre artikler:

* 📊 **[Hvad er kropskomposition?](/kropskomposition/)** – Lær hvordan din krop er opbygget af fedtmasse, muskelmasse og knogler.
* 📏 **[Sådan måler du din fedtprocent](/artikel/udregning-af-fedtprocent/)** – Se de mest almindelige metoder til at estimere din fedtprocent (fx fedttang, scanninger og bioimpedans).
* 📋 **[Normer for fedtprocent for mænd og kvinder](/fedtprocent-normer/)** – Sammenlign dine tal med de anbefalede grænseværdier for din alder og køn.
* 📐 **[Mål din fedtprocent med en fedttang (skinfold)](/skinfold/)** – Prøv vores guide til hudfoldsmålinger og vælg den rigtige formel.

---

## Ofte stillede spørgsmål

{% include motionsplan/faq.html %}

---

<details markdown="1" class="references">
  <summary><h2 id="references">Referencer & Litteratur</h2></summary>

- Ashwell, M., & Hsieh, S. D. (2005). Six reasons why the waist-to-height ratio is a rapid and effective screening tool for obesity and health risks. *Medical Principles and Practice*, 14(3), 138-141.
- Bray, G. A. (2004). Don’t throw the baby out with the bath water. *The American Journal of Clinical Nutrition*, 79(3), 347–349.
- Browning, L. M., Hsieh, S. D., & Ashwell, M. (2010). A systematic review of waist-to-height ratio as a screening tool for the prediction of cardiovascular disease and diabetes: 0.5 could be a suitable global boundary value. *Nutrition Research Reviews*, 23(2), 247–269.
- Butovskaya, M., et al. (2017). Waist-to-Hip Ratio, Body-Mass Index, Age and Number of Children in Seven Traditional Societies. *Scientific Reports*, 7(1), 1–9.
- Dobbelsteyn, C. J., et al. (2001). A comparative evaluation of waist circumference, waist-to-hip ratio and body mass index as indicators of cardiovascular risk factors. *International Journal of Obesity*, 25(5), 652–661.
- Han, T. S., et al. (1995). Waist circumference action levels in the identification of cardiovascular risk factors. *BMJ*, 311(7017), 1401.
- Janssen, I., Katzmarzyk, P. T., & Ross, R. (2004). Waist circumference and not body mass index explains obesity-related health risk. *The American Journal of Clinical Nutrition*, 79(3), 379–384.
- Lee, C. M. Y., et al. (2008). Indices of abdominal obesity are better discriminators of cardiovascular risk factors than BMI: A meta-analysis. *Journal of Clinical Epidemiology*, 61(7), 646–653.
- World Health Organization (WHO). (2011). *Waist circumference and waist-hip ratio: report of a WHO expert consultation, Geneva, 8-11 December 2008*.
</details>
