---
title: "Sådan vælger du det rette styrketræningsprogram"
seo_title: "Guide: Vælg det rigtige styrketræningsprogram (Mål, niveau & dage)"
description: "Er du i tvivl om, hvilket styrkeprogram du skal vælge? Få hjælp til at afklare dit niveau, din træningsfrekvens og find det rette match til din hverdag."
permalink: /vaelg-traeningsprogram/
language: da
header:
  overlay_image: /assets/images/unsplash/photo-1605296867424-35fc25c9212a.jpg
  credit: https://images.unsplash.com/photo-1605296867424-35fc25c9212a
  teaser: /assets/images/unsplash/photo-1605296867424-35fc25c9212a.jpg
  caption: "Sådan vælger du det rette styrketræningsprogram"
category:
  - Styrketræning
tags:
  - styrketræning
  - begynder
  - guide
  - træningsprogrammer
last_modified_at: '2026-08-03T16:00:00Z'
toc: true
faq:
- question: Hvor mange gange om ugen skal jeg træne styrketræning?
  answer: Det kommer an på dit niveau og dine mål. Begyndere opnår fantastiske resultater med 2-3 træninger om ugen, mens øvede ofte træner 3-5 gange om ugen. Det vigtigste er, at du kan restituere mellem træningerne og holde kontinuiteten.
- question: Hvor mange sæt og gentagelser giver bedst resultater?
  answer: Det afhænger af, hvad du vil opnå. Generelt anbefales 3-5 sæt pr. øvelse. For muskelopbygning (hypertrofi) er 6-12 gentagelser effektivt. For styrke ligger man typisk lavere, omkring 1-5 gentagelser med tungere vægte.
- question: Hvilke øvelser bør jeg vælge?
  answer: Fokusér på basisøvelser, der træner flere muskelgrupper ad gangen – fx squat, dødløft, bænkpres, rows og overhead press. De kan suppleres med isolationsøvelser, hvis du har særlige mål eller ønsker mere variation.
- question: Hvor tit skal jeg skifte program?
  answer: Du behøver ikke skifte program ofte, så længe du stadig laver fremskridt. Mange kan følge det samme program i 8-12 uger eller længere. Skift først, når du stagnerer, mister motivationen eller har brug for at ændre fokus.
feature_row_howto_program:
- image_path: /assets/images/unsplash/photo-1526408984842-5f1323d42469.jpg
  credit: https://images.unsplash.com/photo-1526408984842-5f1323d42469
  alt: Hvordan laver jeg mit eget styrketræningsprogram?
  title: Hvordan laver jeg mit eget styrketræningsprogram?
  excerpt: I denne guide viser jeg, hvordan du sammensætter et effektivt styrketræningsprogram. Jeg gennemgår de grundlæggende træningsprincipper, så du kan skabe et program, der er skræddersyet til dine mål og behov.
  url: /traeningsprogram-programlaegning-styrketraening/
  btn_label: Lav dit eget program →
  btn_class: btn--info
---

Er du i tvivl om, hvilket styrketræningsprogram der passer bedst til dig? I denne guide hjælper jeg dig med at finde det rette program – uanset om dit mål er at blive stærkere, få mere muskelmasse eller komme godt i gang som nybegynder.

Sandheden er, at der ikke findes ét "magisk" program, der virker til alle. Det bedste program er det, der **matcher dit fysiologiske niveau, dine mål og din hverdag** – og som du reelt får gennemført uge efter uge.

Her er den enkle tre-trins-raket til at vælge rigtigt første gang.

---

{: .notice--success}
### 📋 Ved du allerede, hvad du leder efter?
Hvis du allerede har styr på dit niveau og din ugentlige frekvens, kan du gå direkte til vores samlede [katalog over gratis styrketræningsprogrammer](/styrketraeningsprogrammer/) og filtrere efter dine behov. Hvis du er på udkig efter programmer til løb, cykling eller andre idrætsgrene, så tjek vores [liste over træningsprogrammer](/traeningsprogrammer/).

---

## Tre-trins-raketten: Sådan træffer du valget

Når du skal vælge træningsprogram, bør du træffe beslutningen ud fra tre helt konkrete trin:

<pre>
 [ TRIN 1: MÅL ]  ──&gt;  [ TRIN 2: NIVEAU ]  ──&gt;  [ TRIN 3: FREKVENS ]
(Styrke vs Muskel)     (Ærlig vurdering)       (Dage i kalenderen)
</pre>

---

## Trin 1: 🎯 Definér dit mål – Vælg træningsretning

Dine muskelceller og dit nervesystem tilpasser sig den belastning, du udsætter dem for. Afklar med dig selv, hvad der er vigtigst for dig lige nu:

<details markdown="1" class="faq">
  <summary>🏋️‍♂️ Styrke (Maksimal muskelstyrke)</summary>

Fokus på at løfte tungere vægte og forbedre maksimal styrke. Du vil typisk træne med lavere reps (1-5) og højere vægt for at maksimere styrken i store løft som squat, dødløft og bænkpres.
</details>

<details markdown="1" class="faq">
  <summary>💪 Muskelmasse (Hypertrofi)</summary>

Målet er at øge størrelsen på dine muskler gennem specifik træning. Dette indebærer træning med moderat til høj volumen (6-12 reps per sæt) og tilstrækkelig udmattelse for at fremme muskelvækst.
</details>

<details markdown="1" class="faq">
  <summary>💥 Kombineret muskelstyrke og muskelvækst (Powerbuilding)</summary>

Dette mål kombinerer elementer fra både muskelmasse (hypertrofi) og muskelstyrke på samme tid. Træningen er en balance, hvor du starter passet med tunge basisløft og afslutter med højere volumen i hjælpeøvelser.
</details>

<details markdown="1" class="faq">
  <summary>🔒 Vedligeholdelse</summary>

Målet er at bevare din nuværende styrke og muskelmasse i travle perioder. Med [Minimum Effective Volume (MEV)](/minimum-effective-volume/) kan du holde dit niveau stabilt uden nødvendigvis at sigte efter nye rekorder.
</details>

---

## Trin 2: 🧱 Vurder dit niveau (Reality-check)

Din [nuværende træningserfaring](/styrketraening/periodisering/) har stor betydning for, hvilket træningsprogram du bør vælge. Dit niveau hænger sammen med, hvor hurtigt du stadig kan opnå fremgang.

Vær helt ærlig over for dig selv – rigtig mange hopper videre til et øvet program alt for tidligt:

<details markdown="1" class="faq" open>
  <summary>🟢 Begynder – Fremgang hver gang du træner</summary>

Som begynder opnår du hurtigt fremgang fra træningspas til træningspas. Din krop reagerer hurtigt på træning, og du behøver ikke kompliceret planlægning. Fokus er på teknik og at opbygge en solid base.

**Kendetegn:**
* Fremgang fra træningspas til træningspas (du kan lægge vægt på næsten hver uge).
* Fokus på at forbedre teknik, styrke og muskelmasse samtidigt.
* Minimal variation i øvelsesvalget.

**Typisk struktur:**
* Samme øvelser og sæt/reps flere gange om ugen (f.eks. et [helkropsprogram](/styrketraening/helkropsprogrammer/)).
* Lineær progression ved hver træning (f.eks. +2,5 kg på stangen).
* Enkel opbygning uden komplicerede deloads.
</details>

<details markdown="1" class="faq">
  <summary>🟡 Øvet – Planlægning og variation for fremgang</summary>

*Er du sikker? Mange springer til øvet, før de har udtømt mulighederne for at få fremgang på simpel lineær periodisering.*

Som øvet har du brug for mere struktureret træning. Du kan ikke længere bare øge vægten hver gang, du træner. I stedet er der brug for planlagt variation i volumen, intensitet og restitution over uger.

**Kendetegn:**
* Fremgang sker på ugentlig basis, ikke fra træning til træning.
* Behov for planlagt variation i intensitet og volumen.
* Periodemæssig fremgang med fokus på én fysisk kvalitet ad gangen (fx styrke eller hypertrofi).

**Typisk struktur:**
* [2-split eller 3-split programmer](/styrketraening/split-programmer/) (f.eks. Push/Pull eller Over/Underkrop).
* Skift mellem tunge og lettere uger.
* Brug af mesocyklusser og struktureret periodisering.
</details>

<details markdown="1" class="faq">
  <summary>🔴 Erfaren – Periodisering og specialisering</summary>

*Er du sikker på, at du er så erfaren, at du har brug for så kompliceret programlægning?*

Som erfaren kræver din træning langsigtet planlægning og specialisering. Du vil ikke længere opnå hurtige gevinster hver uge. I stedet bygger du fremgang op over længere perioder, og din træning skal være mere specifik.

**Kendetegn:**
* Fremgang sker over måneder, ikke uger.
* Behov for høj variation, intensitetsbølger og planlagte deloads.
* Specialiseret mod bestemte discipliner (fx styrkeløft eller bodybuilding).

**Typisk struktur:**
* Makrocyklusser på 8-16 uger eller mere.
* Brug af intensitetsbølger, blokperiodisering og testuger.
* Præcis styring af træningsvolumen tæt på genetisk maks.
</details>

---

## Trin 3: 🗓️ Bestem din træningsfrekvens

Når du skal vælge et program, er et af de vigtigste parametre, hvor mange gange om ugen du gerne vil træne. Husk at det ikke nødvendigvis giver bedre resultater at træne flest mulige dage.

Det allervigtigste i dit programvalg er, at du finder en struktur, som du er motiveret for at holde i din hverdag. Vedholdende træning over tid slår alt andet.

<details markdown="1" class="faq">
  <summary>🗓️ 2-3 dage: Fullbody er det bedste valg</summary>

Hvis du træner 2-3 gange om ugen, er det en stor fordel at vælge et træningsprogram, hvor du træner hele kroppen hver gang – et såkaldt *helkropsprogram*.

Helkropsprogrammet hjælper dig med at holde frekvensen høj nok til, at du får god effekt af træningen. Du kan enten lave den samme træning hver gang, eller skifte mellem 2 variationer (f.eks. Fullbody A og B).

[Se eksempler på Fullbody-programmer →](/styrketraening/helkropsprogrammer/){: .btn .btn--success }
</details>

<details markdown="1" class="faq">
  <summary>📅 3-4 dage: Fullbody eller 2-Split (Upper/Lower / Push/Pull)</summary>

Med tre til fire træninger om ugen har du både mulighed for at fortsætte med et fullbody-program eller begynde på et klassisk 2-split som *Upper/Lower* eller *Push/Pull*.

Fire træningsdage om ugen åbner for 2-split, hvor du skifter mellem overkrop og underkrop. Det giver en fin balance mellem volumen pr. muskelgruppe, intensitet og restitution.

[Se eksempler på 2-Split programmer →](/styrketraening/split-programmer/){: .btn .btn--success }
</details>

<details markdown="1" class="faq">
  <summary>🧩 5+ dage: Avancerede splits og specialisering</summary>

Hvis du gerne vil træne fem eller flere dage om ugen, får du adgang til endnu flere split-muligheder.

Du kan køre et 3-split (f.eks. Push/Pull/Legs) roterende, eller et opdelt fullbody-program som [Jakob Beermans Stærk-program](/artikel/staerk-traeningsprogram/).

Mange vælger 4-split eller 5-split, men for de fleste almindelige motionister giver det sjældent bedre resultater end et gennemtænkt 2-split eller 3-split, da træningsfrekvensen pr. muskelgruppe ofte bliver for lav.
</details>

---

## Særlige overvejelser inden du vælger

### Styrketræningsprogram til kvinder
Der er **ikke fysiologisk forskel** på mænd og kvinders muskler, så træning for mænd og kvinder bør følge de samme grundlæggende principper. 

Ofte kan mænd og kvinder have forskellige ønsker til, hvilke muskelgrupper der prioriteres højest. Kvinder kan vælge frit mellem alle vores [gratis styrketræningsprogrammer](/styrketraeningsprogrammer/) – fokuser på store basisøvelser og tilpas øvelsesvalget til dine specifikke mål.

### Træning i maskiner eller frie vægte
Du kan sagtens bruge et struktureret program, selvom du foretrækker maskiner. Maskiner er fremragende til at opbygge en god grundstyrke med lav teknisk tærskel. De bedste programmer kombinerer dog typisk frie vægte (stang/håndvægte) i starten af træningen med maskiner til sidst.

### Træning uden udstyr (Hjemmetræning)
Har du ikke adgang til et fitnesscenter eller tunge vægte? Du kan stadig opbygge stærke muskler med kropsvægt og enkle redskaber. Tjek vores [komplette guide til hjemmetræning](/hjemmetraening-traeningsprogram/).

---

## Vil du hellere lave dit eget program?

Det kan være en stor fordel at afprøve et par af de [populære styrketræningsprogrammer](/styrketraeningsprogrammer/) først for at mærke på egen krop, hvordan volumen og progression fungerer. Men når du er klar, er det utroligt spændende at sammensætte dit eget program fra bunden.

{% include feature_row type="left" id="feature_row_howto_program" %}

---

## Ofte stillede spørgsmål om valg af styrkeprogram

{% include motionsplan/faq.html %}

---

<details markdown="1" class="references">
  <summary><h2 id="references">Referencer</h2></summary>

- Schoenfeld, B. J., Ogborn, D., & Krieger, J. W. (2016). Effects of resistance training frequency on measures of muscle hypertrophy: a systematic review and meta-analysis. *Sports Medicine*, 46(11), 1689-1697.
- Ochi, E., Maruo, M., Tsuchiya, Y., Ishii, N., Miura, K., & Sasaki, K. (2018). Higher training frequency is important for gaining muscular strength under volume-matched training. *Frontiers in Physiology*, 9.
- Schoenfeld, B. J. (2010). The mechanisms of muscle hypertrophy and their application to resistance training. *The Journal of Strength & Conditioning Research*, 24(10), 2857-2872.
- Schoenfeld, B. J., Grgic, J., & Krieger, J. (2019). How many times per week should a muscle be trained to maximize muscle hypertrophy? A systematic review and meta-analysis of studies examining the effects of resistance training frequency. *Journal of Sports Sciences*, 37(11), 1286–1295.
- Grgic, J., Schoenfeld, B. J., Davies, T. B., Lazinica, B., Krieger, J. W., & Pedisic, Z. (2018). Effect of Resistance Training Frequency on Gains in Muscular Strength: A Systematic Review and Meta-Analysis. *Sports Medicine (Auckland, N.Z.)*, 48(5), 1207–1220.
</details>