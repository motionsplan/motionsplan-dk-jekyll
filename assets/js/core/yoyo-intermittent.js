...
const YYIR1_SHUTTLES = {
  5: 2, 8: 2, 11: 2, 12: 3, 13: 4, 14: 8, 15: 8, 16: 8, 17: 8, 18: 8, 19: 8, 20: 8, 21: 8, 22: 8, 23: 8
};

const YYIR2_SHUTTLES = {
  11: 2, 12: 3, 13: 4, 14: 8, 15: 8, 16: 8, 17: 8, 18: 8, 19: 8, 20: 8, 21: 8, 22: 8, 23: 8, 24: 8
};
...
export const YOYO_INTERMITTENT_FORMULAS = {
  yyir1: {
    ...
    getMaxShuttles: (lvl) => YYIR1_SHUTTLES[lvl] || 8,
  ...
  yyir2: {
    ...
    sd: 3.5,
    sdUnit: 'ml/kg/min',
    maxTestDistance: getMaxIntermittentDistance(YYIR2_SHUTTLES, 11, 24),
    maxTotalShuttles: getMaxIntermittentDistance(YYIR2_SHUTTLES, 11, 24) / 40,
    description: 'Bangsbo formel for veltrænede: Distance × 0.0136 + 45.3.',
    getMaxShuttles: (lvl) => YYIR2_SHUTTLES[lvl] || 8,
...
export function getYoYoIntermittentRating(distance, gender = 'male', age = 20) {
  ...
  if (distance >= matrix[0][ageGroup]) return { label: 'Elite', color: '#1e40af' };
  if (distance >= matrix[1][ageGroup]) return { label: 'Fremragende', color: '#16a34a' };
  if (distance >= matrix[2][ageGroup]) return { label: 'God', color: '#22c55e' };
  if (distance >= matrix[3][ageGroup]) return { label: 'Middel', color: '#eab308' };
  if (distance >= matrix[4][ageGroup]) return { label: 'Under middel', color: '#f97316' };
